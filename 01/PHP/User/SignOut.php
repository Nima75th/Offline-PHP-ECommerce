...
if (isset($_POST['SignOut'])) {
    ...
    $FirstAndLastName = mysqli_real_escape_string($Connection, $_POST['FirstAndLastName']);
    $MobileNumber = mysqli_real_escape_string($Connection, $_POST['MobileNumber']);
    $Address = mysqli_real_escape_string($Connection, $_POST['Address']);

    // اعتبارسنجی
    if (empty($UserName)) array_push($Errors, 'نام کاربری نمیتواند خالی باشد');
    if (empty($Password)) array_push($Errors, 'پسورد نمیتواند خالی باشد');
    if (empty($Email)) array_push($Errors, 'ایمیل نمیواند خالی باشد');
    if (empty($FirstAndLastName)) array_push($Errors, 'نام و نام خانوادگی نمیتواند خالی باشد');
    if (empty($MobileNumber)) array_push($Errors, 'شماره موبایل نمیتواند خالی باشد');
    if (empty($Address)) array_push($Errors, 'آدرس نمیتواند خالی باشد');

    // اگر خطایی نبود
    if (count($Errors) === 0) {

        $Query = mysqli_query($Connection, "INSERT INTO `tblusers` (`UserName`, `Password`, `Email`, `FirstAndLastName`, `MobileNumber`, `Address`) 
                          VALUES ('$UserName', '$Password', '$Email', '$FirstAndLastName', '$MobileNumber', '$Address')");
...
?>
    ...
    <head>
        ...
        <title>فرم افزودن کاربر جدید</title>
    </head>
            ...
            <form action="SignOut.php" method="post" enctype="multipart/form-data">
                ...
                <div class="form-floating">
                    <input
                        type="text"
                        class="form-control"
                        value="<?php echo $FirstAndLastName; ?>"
                        id="floatingInput"
                ...
                <button class="btn btn-primary w-100 py-2" type="submit" name="SignOut">
                    ثبت نام
                </button>
